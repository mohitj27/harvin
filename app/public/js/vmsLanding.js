(function ($) {
  $(function () {
    ...
    $('#harvin-card-panel2').css({
      height: $('#harvin-card-panel')
        .height()
        .toString()
    })
    // $("#navbar").slideUp()
    ...
    $.get(
      '/gallery/category?category[]=vms-main-panel&limit=' + vmsMainPanelLength,
      function (res) {
    ...
    $.get(
      '/gallery/category?category[]=vms-events-panel&limit=' +
        vmsEventPanelLength,
      function (res) {
        if (res.gallery && res.gallery.length > 0) {
