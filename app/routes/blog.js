...
router.get('/all', middleware.isLoggedIn, middleware.isCentreOrAdmin, (req, res, next) => {
  Blog.find({}).sort({ uploadDateUnix: -1 }).exec((err, foundBlog) => {
    if (err)
      console.log(err);
    else {
...
router.post('/', middleware.isLoggedIn, middleware.isCentreOrAdmin, async (req, res, next) => {
  ...
  let blog_name = blogTitle.toLowerCase().replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '_').concat('.html');
  checkBlogDir();
...
router.post('/editmode/:blogTitle/:mode/:check', async (req, res) => {
  let result = await blogCont.updateBlogMode(req.params.blogTitle, req.params.mode, req.params.check);
  res.send(result);
});
