...
router.post(
  ...
  async function (req, res, next) {
    ...
    if (!topicName || validator.isEmpty(topicName)) {
      return errorHandler.errorResponse('INVALID_FIELD', 'topic name', next)
    }
    ...
    try {
      ...
      var updatedTopic
      if (createdFile) {
        updatedTopic = await topicController.addFileToTopicByTopicNameAndUserId(
          newTopic,
          user,
          createdFile
        )
      } else {
        updatedTopic = await topicController.createOrUpdateTopicByTopicNameAndUserId(
          newTopic,
          user
        )
      }

      var updatedChapter = await chapterController.addTopicToChapterByChapterNameAndUserId(
        newChapter,
        user,
        updatedTopic
      )
      var updatedSubject = await subjectController.addChapterToSubjectBySubjectClassAndUserId(
        newSubject,
        className,
        user,
        updatedChapter
      )
      var updatedClass = await classController.addSubjectToClassByClassNameAndUserId(
        newClass,
        user,
        updatedSubject
      )
      updatedSubject = await subjectController.addClassToSubjectById(
        updatedSubject,
        updatedClass
      )
      updatedChapter = await chapterController.addSubjectToChapterById(
        updatedChapter,
        updatedSubject
      )
      updatedTopic = await topicController.addChapterToTopicById(
        updatedTopic,
        updatedChapter
      )

      if (createdFile) {
        await fileController.addTopicChapterSubjectClassToFileById(
          createdFile,
          updatedTopic,
          updatedChapter,
          updatedSubject,
...
router.post(
  ...
  middleware.isCentreOrAdmin,
  async (req, res, next) => {
    res.locals.flashUrl = req.header.referer || '/admin/files/genlink'

    if (!req.files.linkFile) {
      return errorHandler.errorResponse('INVALID_FIELD', 'actual file', next)
    }
    ...
    const date = Date.now()
    const link = {
      linkTitle: req.body.linkTitle.replace(/\s/g, ''),
      addedBy: req.user,
    ...
    if (req.files.cover) {
      link.cover = '/LinkUploads/' + date + '__' + req.files.cover.name
    }

    try {
      await linkController.insertLink(link)
      await fileController.uploadFileToDirectory(
        LINK_GEN + '/' + req.files.linkFile.name,
        req.files.linkFile
      )
      if (req.files.cover) {
        await fileController.uploadFileToDirectory(
          LINK_GEN + '/' + date + '__' + req.files.cover.name,
          req.files.cover
        )
      }
...
router.get('/download/:linkTitle', async function (req, res, next) {
  ...
  try {
    ...
    res.download(
      foundLink.filePath,
      path.parse(foundLink.filePath).base,
      async err => {
        if (err) return next(err || 'Internal Server Error')
        else await linkController.updateDownloadFileByTitle(linkTitle)
      }
    )
  } catch (err) {
    next(err || 'Internal Server Error')
...
router.get('/:fileId', async function (req, res, next) {
  ...
  if (!fileId || !validator.isMongoId(fileId)) {
    return errorHandler.errorResponse('INVALID_FIELD', 'File id', next)
  }
